...
class UpdateFavoritesMoviesPositionUseCaseTest {

    private val repository = mockk<MoviesRepository>()
    private val useCase = UpdateFavoritesMoviesPositionUseCase(repository)
    ...
    fun `invoke should return success when repository returns data`() = runBlocking {
        // --- ARRANGE ---
        coEvery { repository.updateMoviePosition(fakeMovies) } returns Unit

        // --- ACT ---
        val result = useCase(fakeMovies).first()
    ...
    fun `invoke should return failure when repository throws exception`() = runBlocking {
        ...
        coEvery { repository.updateMoviePosition(fakeMovies) } throws expectedError
